...
task sobelX(r_image    : region(ispace(int2d), Pixel),
            r_interior : region(ispace(int2d), Pixel))
where
  reads(r_image.smooth), writes(r_interior.gradient)
do
  var ts_start = c.legion_get_current_time_in_micros()
  for e in r_interior do
    var gx: double =
       0.0*r_image[e].smooth         +
       0.0*r_image[e+{ 0, 1}].smooth +
       0.0*r_image[e+{ 0,-1}].smooth +
    ...
    r_interior[e].gradient.x = [double](gx)
  end
  var ts_end = c.legion_get_current_time_in_micros()
  ...
  for e in r_interior do
    var gy: double =
      ...
      -2.0*r_image[e+{ 0,-1}].smooth +
       ...
       r_interior[e].gradient.y = [double](gy)
  end
  var ts_end = c.legion_get_current_time_in_micros()
...
task suppressNonmax(r_image    : region(ispace(int2d), Pixel),
                    r_interior : region(ispace(int2d), Pixel))
where
  reads(r_image.gradient), writes(r_image.local_maximum)
do
  var ts_start = c.legion_get_current_time_in_micros()
  for e in r_interior do

    var T: double = cmath.atan2(r_image[e].gradient.y,r_image[e].gradient.x)
    T = T/(PI/4.0)
    T = cmath.round(T)
    if T < 0 then
       T = T + 4
    ...
    var a: double[3]
    if T == 0 or T == 4 then
      a = array(r_image[e+{-1, 0}].gradient:norm(),r_image[e].gradient:norm(),r_image[e+{1,0}].gradient:norm())
    elseif T == 1 then
      a = array(r_image[e+{-1,-1}].gradient:norm(),r_image[e].gradient:norm(),r_image[e+{1,1}].gradient:norm())
    elseif T == 2 then
      a = array(r_image[e+{ 0,-1}].gradient:norm(),r_image[e].gradient:norm(),r_image[e+{0,1}].gradient:norm())
    elseif T == 3 then
      a = array(r_image[e+{-1, 1}].gradient:norm(),r_image[e].gradient:norm(),r_image[e+{1,-1}].gradient:norm())
    end
    if a[1] < a[0] or a[1] < a[2] then
       r_image[e].local_maximum = false
    end
  end
  var ts_end = c.legion_get_current_time_in_micros()
